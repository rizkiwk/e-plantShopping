...
function ProductList({ onHomeClick }) {
    ...
    const renderPlantCategory = (index, item) => {
        return(
            <div key={index}>
                <h1>{item.category}</h1>
                <div className="product-list">
                    {item.plants.map((plant, keyItem) => (renderPlantItem(keyItem, plant))) }
                </div>
            </div>
        );
    }

    const renderPlantItem = (index, item) => {
        return (
        ...
        <div>
            ...
            {!showCart ? (
                <div className="product-grid">
                    {plantsArray.length > 0 && plantsArray.map((item, index) => (renderPlantCategory(index, item)))}
                </div>
            ) : (
